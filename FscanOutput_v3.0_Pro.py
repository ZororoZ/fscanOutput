...
#输出NetBios信息
def NetBios(datalist):

    sheetList = [['Ip', 'NetBios']]
    pattern = r'(.*NetBios.*)'
    ...
    for i in info_n:
        ...
        netbios_get = re.findall(r'.*NetBios.*', i)
        netbios = netbios_get[0]
        ip.append(netbios)
        sheetList.append(ip)

    OutPut('NetBios', sheetList)
...
def OutPut(sheetname,sheetList):
    ...
    delimiter = "================{}================\n"
    output_path = os.path.join(output_folder, f"{input_filename}_{sheetname}.txt")
...
============================================================
    ...
    print("[+++]成功识别NetInfo共计：%s 个" % (wt7.max_row - 1))
    wt7 = New_fscanxlsx['NetBios']
    print("[+++]成功识别NetBios共计：%s 个\n" % (wt7.max_row - 1))
    print("+---------------------------------+\n")

    print(f'[+]结果已经整理输出至 -- {input_filename} -- 文件夹下！\n')
